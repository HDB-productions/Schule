(function () {
  ...
  function sanitizeStore(candidate) {
    ...
    exams = exams.map(normalizeExamRecord);
  ...
  function duplicateExamRecord(exam) {
    ...
    return {
      ...
      printLayout: remapPrintLayout(exam.printLayout, structureIdMap, studentIdMap)
    };
  }
  ...
  function importExamPayload(raw) {
    ...
    return normalizeExamRecord(parsed);
  }
  ...
  var ui = {
    ...
    editingNodeId: null,
    collapsedNodeIds: {},
    printPreviewSelectedRowKey: null
  };
  ...
  function renderTabs() {
    var tabs = [
      ...
      { id: "printLayout", label: "Druckvorschau" },
      ...
      return `
        <aside class="subpanel print-layout-sidebar">
          <div class="print-layout-empty">
            <h3>Layouter</h3>
            <p class="muted-text">F\u00fcr die Vorschau gibt es noch keine druckbaren Elemente.</p>
          </div>
        </aside>
      `;
    }
    ...
    return `
      <aside class="subpanel print-layout-sidebar">
        <div class="stack-gap">
          <div>
            <h3>Layouter</h3>
            <p class="muted-text">Die Einstellungen gelten f\u00fcr alle Sch\u00fclerb\u00f6gen. Gezeigt wird ${escapeHtml(previewStudent.displayName)}.</p>
          </div>
          <dl class="compact-definition-list">
      ...
      return `
        <section class="panel stack-gap print-layout-view">
          <div class="panel-header">
            <div>
              <h2>Druckvorschau</h2>
              <p class="muted-text">Lege zuerst mindestens einen aktiven Sch\u00fcler an, damit die Vorschau aufgebaut werden kann.</p>
            </div>
          </div>
          <div class="print-layout-empty">
            <p class="muted-text">Ohne aktiven Sch\u00fcler kann der Layouter keine Seite rendern.</p>
          </div>
        </section>
      `;
    }
    ...
    return `
      <section class="panel stack-gap print-panel print-live-preview">
        <div class="panel-header no-print">
          <div>
            <h2>Druckansicht</h2>
            <p class="muted-text">Druckfertige Sammelausgabe. Feinjustierung f\u00fcr Umbr\u00fcche und Abst\u00e4nde erfolgt in der Druckvorschau.</p>
          ...
          <div class="inline-actions wrap-actions">
            <button type="button" class="secondary-button" data-action="switch-tab" data-tab="printLayout">Druckvorschau \u00f6ffnen</button>
  ...
  function removeNode(nodeId) {
    ...
    mutateCurrentExam(function (exam) {
      ...
      prunePrintLayoutRowsForNodeIds(exam, ids);
      Object.keys(exam.evaluations).forEach(function (key) {
