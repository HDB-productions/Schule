...
const lab=moduleCode('dynamot-labor-szene.js','LabScene',{'./dynamot-3d-vendor/three.module.js':'threeModule','./dynamot-3d-vendor/OrbitControls.js':'orbitModule','./dynamot-3d.js':'Dynamot3D'});
const license=fs.readFileSync(path.join(dir,'dynamot-3d-vendor/LICENSE'),'utf8');
const bundle=('/* Three.js license\n'+license+'\n*/\n'+[core,three,orbit,view,lab].join('\n')).replace(/<\/script/gi,'<\\/script');
const source=fs.readFileSync(path.join(dir,'dynamot-labor.quelle.html'),'utf8');
const learning=['dynamot-aufgaben.js','dynamot-diagramm.js','dynamot-lern-ui.js'].map(f=>fs.readFileSync(path.join(dir,f),'utf8')).join('\n').replace(/<\/script/gi,'<\\/script');
const styles=['dynamot-lern-ui.css','dynamot-diagramm.css'].map(f=>fs.readFileSync(path.join(dir,f),'utf8')).join('\n');
const network=fs.readFileSync(path.join(dir,'dynamot-stromnetz.js'),'utf8');
fs.writeFileSync(path.join(dir,'dynamot-labor.html'),source.replace('/* DYNAMOT_3D_BUNDLE */',()=>bundle).replace('/* DYNAMOT_LEARNING_CSS */',()=>styles).replace('/* DYNAMOT_NETWORK_MODEL */',()=>network).replace('/* DYNAMOT_LEARNING_BUNDLE */',()=>learning));
