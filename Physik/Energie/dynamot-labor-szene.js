...
export function createFlowTracker(){
 const phases=new Map();let lastTime=null;
 return {update(wires,time=0){
  ...
  for(const wire of wires){const old=phases.get(wire),I=wire.current||0;
   const delta=old===undefined||Math.abs(I)<=.0001?0:-Math.sign(I)*Math.min(1.8,.25+Math.abs(I)*1.2)*dt;
   phases.set(wire,(((old||0)+delta)%1+1)%1);
  }
 },phase(wire){return phases.get(wire)||0;}};
}
...
export function createLabScene(container,{onPick,onHover,onViewChange}={}){
 ...
 const orbit=new OrbitControls(camera,canvas);orbit.target.set(0,.2,1);orbit.enablePan=true;orbit.screenSpacePanning=true;orbit.touches.ONE=THREE.TOUCH.ROTATE;orbit.touches.TWO=THREE.TOUCH.DOLLY_PAN;orbit.minDistance=9;orbit.maxDistance=35;orbit.maxPolarAngle=Math.PI*.49;
 ...
 const targets=[],models=new Map(),slotMarks=new Map();let current={devices:[],wires:[]},pending=null,tool=null,selected=null,selectedWire=-1;
 ...
 function rebuildWires(){disposeTree(flowGroup);flowGroup.clear();disposeTree(wiresGroup);wiresGroup.clear();wireCurves.clear();wireEnds.clear();topPorts.clear();scene.updateMatrixWorld(true);const counts=new Map();for(let i=0;i<current.wires.length;i++){const w=current.wires[i],ends=[];w.color??=i%2?0x285375:0xb43c35;for(const [id,pin]of [[w.a,w.ap],[w.b,w.bp]]){const key=id+':'+pin,stack=counts.get(key)||0;counts.set(key,stack+1);ends.push(plug(w,id,pin,stack,w.color));}wireEnds.set(w,ends);}const duplicates=new Map();
  for(let i=0;i<current.wires.length;i++){
   ...
   for(let j=0;j<4;j++){const electron=mesh(new THREE.SphereGeometry(.085,10,8),mat(0x35a6ff,{emissive:0x128dff,emissiveIntensity:2.5,roughness:.2}),flowGroup);electron.userData={wire:i,phase:j/4};electron.visible=false;}

  }
 }
 function update(state){current=state;flowTracker.update(state.wires,state.simulationTime||0);for(const electron of flowGroup.children){const w=state.wires[electron.userData.wire],I=w?.current||0;electron.visible=state.electronFlow===true&&Math.abs(I)>.0001;if(electron.visible){const curve=wireCurves.get(w),t=(electron.userData.phase+flowTracker.phase(w))%1;electron.userData.t=t;if(curve)electron.position.copy(curve.getPointAt(t));else electron.visible=false;}}for(const d of state.devices){const m=models.get(d.id);if(!m)continue;if(d.type==='motor'){m.setCrankAngle(d.angle*Math.PI/180);if(d.weight){const y=(-2.46+d.height*1.3-1.55)/.72,rope=m.parts.weightRig.children[0],weight=m.parts.weightRig.children[1];weight.position.y=y;rope.position.y=(y+.2)/2;rope.scale.y=Math.abs(y+.2)/2.25;}}else{const level=(d.power||0)>.0001?Math.pow(Math.min(1,d.power/3),.55):0;m.bulb.material.opacity=.35+level*.6;m.bulb.material.color.setHex(level>0?0xfff18c:0x899b98);m.bulb.material.emissiveIntensity=level*8;m.filament.material.emissiveIntensity=level*16;m.halo.material.opacity=level>0?.25+level*.7:0;m.halo.scale.setScalar(1.2+level*1.4);}}
 ...
 render();}
 function render(){for(const o of wiresGroup.children){const i=o.userData.pick?.index;if(Number.isInteger(i)){o.material.color.setHex(i===selectedWire?0xf0ad26:current.wires[i].color);o.material.emissive.setHex(i===selectedWire?0x7a4e00:0);}}renderer.render(scene,camera);}
 function pickAt(clientX,clientY){const r=canvas.getBoundingClientRect(),mouse=new THREE.Vector2((clientX-r.left)/r.width*2-1,-(clientY-r.top)/r.height*2+1),ray=new THREE.Raycaster();ray.setFromCamera(mouse,camera);const hits=ray.intersectObjects([...targets,...wiresGroup.children],false);const port=hits.find(h=>h.object.userData.pick?.kind==='port');if(port)return port.object.userData.pick;let near=null,best=12;for(let i=0;i<current.wires.length;i++){const curve=wireCurves.get(current.wires[i]);if(!curve)continue;let x=projectPoint(curve.getPointAt(0));for(let j=1;j<=80;j++){const y=projectPoint(curve.getPointAt(j/80)),dx=y.x-x.x,dy=y.y-x.y,len=dx*dx+dy*dy,t=Math.max(0,Math.min(1,((clientX-x.x)*dx+(clientY-x.y)*dy)/(len||1))),distance=Math.hypot(clientX-x.x-t*dx,clientY-x.y-t*dy);if(distance<best){best=distance;near={kind:'wire',index:i};}x=y;}}if(near)return near;return hits.find(h=>h.object.userData.pick)?.object.userData.pick||null;}
 ...
 resetView();return {canvas,rebuild,update,pickAt,getViewState,restoreView,projectWire(index,t){const curve=wireCurves.get(current.wires[index]);return curve?projectPoint(curve.getPointAt(t)):null;},getPlugSamples(){return [...wireEnds.values()].flat().map(p=>({id:p.id,pin:p.pin,stack:p.stack,exit:p.exit.toArray(),back:p.back.toArray(),axis:p.axis.toArray(),side:p.side.toArray()}));},getWireLift(index){return wireCurves.get(current.wires[index])?.liftHeight||0;},getWirePoints(index,count=100){const curve=wireCurves.get(current.wires[index]);return curve?curve.getSpacedPoints(count).map(p=>({x:p.x,y:p.y,z:p.z})):[];},getFlowSamples(){return flowGroup.children.map(e=>({wire:e.userData.wire,t:e.userData.t,visible:e.visible}));},setView,resetView,setMode(t,p,s,w=-1){tool=t;pending=p;selected=s;selectedWire=w;update(current);},projectPort(id,pin){scene.updateMatrixWorld(true);const p=topPorts.get(id+':'+pin)||worldPort(id,pin);return p&&projectPoint(p);},projectDevice(id){const m=models.get(id);if(!m)return null;scene.updateMatrixWorld(true);return projectPoint(m.group.localToWorld(new THREE.Vector3(0,1,0)));},projectAccessory(id){const m=models.get(id),d=current.devices.find(d=>d.id===id);if(!m||!d||!d.crank&&!d.weight)return null;scene.updateMatrixWorld(true);const part=d.crank?m.parts.crank:m.parts.weightRig.children[1];return projectPoint(part.getWorldPosition(new THREE.Vector3()));},projectSlot(id){const s=LAB_SLOTS.find(v=>v.id===id);return projectPoint(new THREE.Vector3(s.x,.32,s.type==='motor'?4.8:s.z));},zoom(factor){camera.position.sub(orbit.target).multiplyScalar(factor).add(orbit.target);orbit.update();render();onViewChange?.(getViewState());},dispose(){observer.disconnect();orbit.dispose();disposeTree(scene);renderer.dispose();renderer.forceContextLoss();canvas.remove();}};
}
