...
fs.writeFileSync(path.join(dir,'dynamot-labor.html'),source.replace('/* DYNAMOT_ENERGY_MODEL */',()=>['dynamot-energie.js','dynamot-energie-puffer.js'].map(f=>fs.readFileSync(path.join(dir,f),'utf8')).join('\n')).replace('/* DYNAMOT_3D_BUNDLE */',()=>bundle).replace('/* DYNAMOT_LEARNING_CSS */',()=>styles).replace('/* DYNAMOT_NETWORK_MODEL */',()=>network).replace('/* DYNAMOT_LEARNING_BUNDLE */',()=>learning));
