...
(function(root){
 ...
 const lossFraction=b=>Number.isFinite(b?.fraction)?Math.max(.05,Math.min(.95,b.fraction)):.25;
  function flowLayout(main,branches=[]){
   const cell=82,top=75,base=66,height=266,width=main.length*cell;
   ...
   for(let i=0;i<main.length-1;i++){
    ...
    if(i%2&&isConverter(main[i])){
     const branch=byFrom.get(i),fraction=branch?lossFraction(branch):0,heat=incoming*fraction,outgoing=incoming-heat,x0=(i+1)*cell,radius=Math.min(70,Math.max(52,heat+4));
 ...
 function mountLosses(host,{length=5,value,onChange=()=>{},readonly=false}={}){
  ...
  function render(){
   const scroll=host.querySelector('.de-losses-scroll')?.scrollLeft||0,layout=flowLayout(data.main,data.branches);
  ...
  function fractionAt(from,clientY){const s=flowLayout(data.main,data.branches).splits.find(s=>s.from===from),r=host.querySelector('.de-sankey-svg').getBoundingClientRect();return 1-(clientY-r.top-s.top)/s.incoming;}
