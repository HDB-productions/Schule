...
export function createLabScene(container,{onPick,onHover,onViewChange}={}){
 ...
 const energyView=createEnergyView(energyGroup,{models,wireCurves,worldPort,camera});let current={devices:[],wires:[]},pending=null,tool=null,selected=null,selectedWire=-1;
 ...
 orbit.addEventListener('change',()=>{scene.updateMatrixWorld(true);energyView.update(current);render();});orbit.addEventListener('end',()=>onViewChange?.(getViewState()));
