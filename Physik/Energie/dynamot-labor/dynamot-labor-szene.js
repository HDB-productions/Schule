...
export function createLabScene(container,{onPick,onHover,onViewChange}={}){
 ...
 function rebuild(state){current=state;energyView.reset();for(const [id,m]of models){devicesGroup.remove(m.group);disposeTree(m.group);}models.clear();targets.splice(LAB_SLOTS.length); // slots are the initial raycast targets
