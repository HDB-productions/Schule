...
- Ein korrekt geprüfter Aufbau bleibt gesperrt. Start/Pause, Antreiben, Drehzahl, Masse, Gewicht hoch- und heruntersetzen sowie Lampeneigenschaften bleiben bedienbar. Im Versuch mit zwei Kurbeln erlaubt „Polung umkehren“ den erforderlichen kontrollierten Buchsentausch. Das Freie Labor bleibt uneingeschränkt.
...
Mehrere violette Pakete um das Gewicht stellen dessen endlichen Lageenergievorrat dar. Bei gleicher Höhe bleibt ihre Anzahl gleich; eine größere Masse vergrößert ihre Fläche statt ein einzelnes Riesensymbol zu erzeugen. Beim Fallen wird der Vorrat kleiner, beim Heben größer. Der Vorrat bleibt auch ohne Bewegung sichtbar. Die bewegten Symbole folgen dem momentanen Energiefluss: Sie erscheinen sofort auf aktiven Wegen und verschwinden bei physikalischem Stillstand sofort. Pause friert die Darstellung ein. Manuelles Hoch- und Heruntersetzen passt den dargestellten Vorrat unmittelbar an.
...
### Energieflussdiagramm in zwei Lernstufen

Zuerst erstellen die Lernenden nur die Hauptkette. Die Anweisung fordert ausdrücklich dazu auf, thermische Energie zunächst zu ignorieren. Zusätzliche Abzweige sind in dieser Stufe nicht verfügbar; eine thermische Energieform innerhalb der Hauptkette wird als falsch bewertet.

Nach erfolgreicher Prüfung folgt die Vertiefung: Die Lernenden beobachten die Energieanzeige und bestimmen selbst, welche weitere Energieform abgegeben wird. Sie ziehen diese auf den Ausgangspfeil eines Wandlers oder wählen sie per Antippen aus. Der weiterführende Anteil und der nach unten abknickende thermische Anteil bilden ein Sankey-Diagramm. Eine direkt auf der Trennlinie liegende Griffstelle lässt sich mit Maus, Touch oder Pfeiltasten verschieben. Alle folgenden Pfeildicken werden aus dem jeweils verbleibenden Anteil berechnet. Eingangsbreite = weitergeführte Breite + abgegebene Breite. Die Anteile sind frei zwischen 5 und 95 Prozent einstellbar; es gibt keine behaupteten Messwerte oder vorgegebene Wirkungsgradlösung. Das Gewicht bleibt eine idealisierte Speicherstation ohne eigenen thermischen Abzweig.

Die vergrößerbare Diagrammansicht bietet zusätzlichen Platz; auf schmalen Bildschirmen bleibt die Kette horizontal verschiebbar. Nach dem Ergänzen aller Abgänge klappt die Energiepalette ein, damit die Aufteilung sichtbar bleibt. Im Nachschlagewerk kann die Aufteilung wieder zum Untersuchen geöffnet werden.

Die bisherigen Aufgaben-Signaturen und 20 Punkte bleiben unverändert. Die neue Vertiefung erhält keine zusätzlichen Punkte; ihr Entwurf, ihre Anteile und ihr Abschluss werden separat unter `diagramExtensions` im bestehenden Aufgabenstand gespeichert. Bereits bestätigte alte Diagramme und Antworten werden nicht neu bewertet oder überschrieben. Die Vertiefung kann auch für diese Diagramme nachgeholt werden. Alte thermische Abzweige werden für die neue Ansicht übernommen; fehlen Anteile, dient 25 Prozent als veränderbarer Beispielwert. Angepasste ältere unbestätigte Entwürfe mit Abzweigen bleiben zusätzlich als Diagramm-Archiv erhalten.

Aktuelle Prüfungen: `dynamot-diagramm-stufen.test.cjs` prüft Fachlogik und Erweiterungsdaten, `dynamot-diagramm-editor.test.cjs` Geometrie und Maus-/Touch-/Tastaturbedienung, `dynamot-diagramm-stufen-browser.test.cjs` den vollständigen zweistufigen Ablauf mit E1, Wiederherstellung und Altständen. Der allgemeine aktuelle Ablauf steht in `dynamot-ablauf-browser.test.cjs`. Der ältere `dynamot-labor.test.cjs` bildet noch den historischen Ablauf vor getrennten Versuchen ab und ist kein aktueller Gesamtabnahmetest. Der Palette-Test nutzt den historischen Editormodus ausdrücklich.

### Alte Speicherstände und einzelner Neustart

Beim Laden wird nach der Prüfung des Aufgabenstands jeder bereits begonnene, über den Bauschritt hinaus bestätigte Versuch mit seinem Labor-Modus abgeglichen. Das gilt auch, wenn `labModes` bereits vorhanden, aber leer oder ungesperrt ist. Ein passender vorhandener Aufbau bleibt erhalten; andernfalls wird der gespeicherte geprüfte Aufbau verwendet. Fehlt ein nutzbarer Aufbau (beispielsweise alte Tischplätze), wird die heutige Versuchsvorlage rekonstruiert. Der Aufbau ist danach gesperrt, die Bauvorlage ausgeblendet. Antworten, Diagramme und erworbene Punkte werden dabei nicht gelöscht. Unlesbare Speicherstände bleiben weiterhin vor Überschreiben geschützt.

In jedem Einzelversuch gibt es „Diesen Versuch erneut durchführen“. Nach Öffnen der Erläuterung setzt die Schaltfläche nur diesen Versuch zurück: Aufbau, Beobachtungsnachweise, Antworten, Diagramme samt Vertiefung, Notizen und erworbene Punkte (höchstens fünf). Andere Versuche und das freie Labor bleiben erhalten. Der Neustart wird in E1 gespeichert und die zugehörigen E2-Punkte werden geleert.

Prüfung: `tests/dynamot-modus-wiederherstellung.test.cjs` prüft teilweise migrierte Modi, fehlende/alte Aufbauaufzeichnungen, Sperre und ausgeblendete Bauvorlage, Einzel-Reset, E1-Wiederöffnung und isolierte E2-Punkte. `tests/dynamot-reset.test.cjs` prüft den Engine-Reset einschließlich Teilständen und unveränderten anderen Versuchen.

Die Energieanzeige verwendet für alle aktiven Wege dieselbe ruhige, schematisch verlangsamte Transportgeschwindigkeit. Sie bildet den momentanen Energiefluss ab: Bereits beim Betriebsbeginn sind Symbole auf den aktiven Wegen verteilt; einen zusätzlichen Anlauf oder Nachlauf gibt es nicht. Energiemenge und Leistung werden über Symbolflächen statt über die Bewegungsgeschwindigkeit veranschaulicht. An Generator, Motor, Lampe und Gewicht schrumpft das Eingangssymbol unmittelbar neben den wachsenden Ausgangssymbolen; anschließend entfernen sich diese. Das gilt auch bei mehreren Quellen und Verbrauchern. Am Gewicht geht grün dargestellte mechanische Energie in den violetten Lageenergie-Vorrat über. Diese Zeitlupe verändert weder die physikalische Simulation noch Aufgaben oder Punkte.

„Gewicht heruntersetzen“ stellt das Gewicht von Hand auf Höhe null und setzt die Drehzahl zurück. Dadurch kann auch ein bereits bestätigter Hebeversuch erneut gestartet werden, ohne seinen Aufbau oder seine Punkte zu löschen. Die manuelle Lageänderung startet den dargestellten Vorrat passend zur neuen Höhe.

Die reine Animationslogik liegt in `dynamot-energie-puffer.js`. Freie Pakete legen pro Simulationssekunde 1,6 Szeneneinheiten zurück; der Bezugspunkt bewegter Bauteile bewegt sich zusätzlich. Neue Wege werden einmal mit periodischen Paketen vorbelegt, danach werden zurückgelegte Strecken und Ankunftsereignisse fortgeschrieben. Eine Umwandlung endet vor der nächsten Ankunft derselben Spur. Bei mehreren Kabeln behalten die Ankünfte eigene, nebeneinanderliegende Umwandlungsstellen. Die farbigen Rechtecke berühren den Eingang in der Blickebene auch beim Drehen der Kamera. Die Umwandlungen sind kein zusätzlicher realer Energiespeicher und bilden keine verzögerte Gesamt-Joule-Buchhaltung.

Bei zwei handbetriebenen Kurbeln berücksichtigt der Energieplan auch Einspeisung von beiden Kabelenden: Entgegengesetzte gleich große Generatorspannungen können Kabelwärme erzeugen, obwohl zwischen den Geräten kein Netto-Energietransfer stattfindet. Gleiche Spannungen führen dagegen zu keinem Kabelstrom; die Hände decken die Reibung ihrer Geräte. Ungleiche Drehzahlen können Energie zum zweiten Gerät und sogar an dessen haltende Hand übertragen. Die Kabelpolung entscheidet mit über diese Fälle. `plan.wires[].flows` unterscheidet gerichteten Transfer und Beiträge zur Kabelwärme; die bisherigen Routingfelder bleiben erhalten.

Prüfung: `tests/dynamot-energie-puffer.test.cjs` für Transport, direkte Ankunft, Vorbelegung und Stillstand; `tests/dynamot-gegenkurbeln.test.cjs` für die physikalischen Bilanzen beider Drehrichtungen und Polungen; `tests/dynamot-energie-browser.test.cjs` für die vier realen Versuchsaufbauten, Kantenkontakt, Höchstlast und Vorrat; `tests/dynamot-gewicht-hoehe.test.cjs` für die Höhenbedienung.
