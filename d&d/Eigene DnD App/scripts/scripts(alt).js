...
function applyHpChange (change) {
  const currentHpEl = document.getElementById('currentHp') // Aktuelle HP auslesen
  let currentHp = parseInt(currentHpEl.textContent, 10) // parseInt wandelt den String in eine Ganzezahl um
  currentHp += change // Änderung der HP summiert auf die aktuellen HP
  const maxHp = parseInt(document.getElementById('maxHp').textContent, 10) // Maximale HP auslesen
  currentHp = Math.max(0, Math.min(currentHp, maxHp)) // HP-Wert wird auf 0 bis maxHp begrenzt
  updateHealthBar(currentHp, maxHp) // HP-Leiste wird aktualisiert
}
...
/*  loadCharacter  => Lädt die Charakterdaten in das UI
      @param {Object} allData - JSON-Daten in denen die Charakterdaten enthalten sind
      @param {string} characterName - Name des Charakters*/
function loadCharacter (allData, characterName) {
  const sc = allData.Charaktäre.Spielercharaktere //Definiert sc als Spielercharaktere in form einer Liste
